...
function TopCartButton({
  className = 'text-[#1C1B1B]',
  iconFill = '#1C1B1B',
  badgeClassName = 'bg-[#1C1B1B] text-white'
}) {
  ...
  return (
    <button
      ...
      className={`relative flex h-8 w-8 items-center justify-center transition-all hover:scale-110 ${className}`}
      aria-label="Open cart"
    >
      <svg viewBox="0 0 24 24" className="block h-6 w-6" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
        <path
      ...
      {cartCount > 0 ? (
        <span className={`absolute right-0 top-0 flex h-[14px] min-w-[14px] items-center justify-center rounded-full px-1 text-center text-[8px] font-semibold leading-none ${badgeClassName}`}>
